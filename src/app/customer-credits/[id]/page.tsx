  ...
  return (
    <DashboardLayout>
      <div className="max-w-5xl mx-auto space-y-5 sm:space-y-6">
        ...
        <div className="bg-white border border-[#EDE4D5] rounded-2xl sm:rounded-3xl p-4 sm:p-8 shadow-sm space-y-5 sm:space-y-6 print:border-none print:shadow-none">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 bg-[#FAF6F0] p-4 sm:p-5 rounded-2xl border border-[#EDE4D5]">
            <div className="space-y-1.5">
              <span className="text-[10px] sm:text-[11px] font-black uppercase tracking-wider text-[#8C7361] block">
                Customer Details
              </span>
              <div className="flex items-center gap-2 text-[#2C1B10] font-extrabold text-base sm:text-lg">
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex items-center gap-2 text-[#2C1B10] font-bold text-xs sm:text-sm">
                ...
                <span>Issued Date: {credit.date ? format(new Date(credit.date), "MMMM d, yyyy") : format(new Date(credit.createdAt), "MMMM d, yyyy")}</span>
              </div>
              {parsed.notes && (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
            <div className="bg-[#FAF6F0] border border-[#EDE4D5] rounded-2xl p-4">
              <span className="text-[11px] sm:text-xs font-bold uppercase text-[#8C7361] block">Total Product Credit</span>
            ...
            <div className="bg-emerald-50/60 border border-emerald-200 rounded-2xl p-4">
              <span className="text-[11px] sm:text-xs font-bold uppercase text-emerald-800 block">Total Settlement Paid</span>
            ...
            <div className="bg-rose-50/60 border border-rose-200 rounded-2xl p-4">
              <span className="text-[11px] sm:text-xs font-bold uppercase text-rose-800 block">Remaining Balance Due</span>
          ...
          <div className="space-y-3">
            <h3 className="text-xs sm:text-sm font-extrabold text-[#2C1B10] uppercase tracking-wider flex items-center gap-2">
              ...
              <span>Products Taken on Credit</span>
            </h3>
                  ...
                  {parsed.items.map((item, idx) => (
                    <div key={idx} className="bg-[#FAF6F0] p-3 rounded-xl border border-[#EDE4D5] space-y-1.5">
                      ...
                      <div className="flex items-center justify-between text-[11px] text-[#8C7361] pt-1 border-t border-[#EDE4D5]/60">
                        <span>Quantity: <strong className="font-mono text-[#4A2E1B]">{item.qty}</strong></span>
                        <span>Unit Price: <span className="font-mono text-[#4A2E1B]">{item.unitPrice}</span></span>
                      </div>
                    </div>
                  ))}
                </div>
          ...
          <div className="space-y-3 pt-2">
            <h3 className="text-xs sm:text-sm font-extrabold text-[#2C1B10] uppercase tracking-wider flex items-center gap-2">
              ...
              <span>Repayment Settlement History Ledger</span>
            </h3>
                  ...
                  {paymentLedger.map((p) => (
                    <div key={p.id} className="bg-[#FAF6F0] p-3 rounded-xl border border-[#EDE4D5] space-y-2">
                      ...
                      <div className="flex items-center justify-between text-xs pt-1.5 border-t border-[#EDE4D5]">
                        <div>
                          <span className="text-[10px] uppercase font-bold text-[#8C7361] block">Paid Amount</span>
          ...
          <DialogContent className="max-w-md rounded-2xl p-5 sm:p-6">
            <DialogHeader>
              <DialogTitle className="text-base sm:text-lg font-extrabold text-[#2C1B10] flex items-center gap-2">
                ...
                <span>Record Credit Settlement / Payment</span>
              </DialogTitle>
            </DialogHeader>
            <form onSubmit={handlePay} className="space-y-4 mt-2">
              ...
              <div>
                <label className="text-xs font-bold text-[#2C1B10] mb-1.5 block uppercase">Payment Amount (ETB)</label>
                <Input
                  type="number"
                  step="any"
                  required
                ...
                />
                <p className="text-[11px] text-[#8C7361] mt-1">Enter any payment amount up to {remainingBalance.toFixed(2)} ETB</p>
              </div>

              <div>
                <label className="text-xs font-bold text-[#2C1B10] mb-1.5 block uppercase">Payment Date</label>
                <Input
                  type="date"
                  required
                  value={paymentDate}
